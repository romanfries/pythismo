...
VAR_MOD_INFORMED = torch.tensor([0.15, 0.3, 0.6], device=DEVICE)
...
VAR_LIKELIHOOD_TERM = [0.2, 0.6, 1.0, 1.4, 1.8, 2.2, 2.6, 3.0]
GAMMA = 50.0
...
def mcmc_task(gpu_id, chunk):
    os.environ['KMP_DUPLICATE_LIB_OK'] = 'True'
    torch.cuda.set_device(gpu_id)
    device_ = torch.device(f'cuda:{gpu_id}')
    ...
    for var_likelihood, percentage in chunk:
        # for l_ in range(len(meshes)):
        for l_ in range(1):
            target = meshes[l_]
            del meshes[l_]
